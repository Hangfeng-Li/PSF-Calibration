...
coeff_xy=np.zeros((32,32,23,64))
for z1 in range(23):
    for j1 in range(32):
        for i1 in range(32):
            coeff_xy[i1,j1,z1,:]=coeff[j1,i1,z1,:]
...
# init_parameters=[12.07357434,14.41640559,242.60563827,4.0065658,15.06827696]
init_parameters=[13,13,20000,4,16.7]
# init_parameters=[13.5,13.8,20000,5,16.7]
...
def MLELM():	
    ...
    newTheta=init_parameters.astype(float)
    oldTheta=init_parameters.astype(float)
    ...
    for ii in range(sz):
        for jj in range(sz):
            model1,newDudt=DerivativeSpline_v3(ii,jj,xc,yc,zc,spline_xsize,spline_ysize,spline_zsize,delta_f,delta_dxf,delta_dyf,delta_dzf,coeff_xy,newTheta,newDudt,off,xstart,ystart,zstart)
            model=model1
...
# 					memset(hessian,0,NV*NV*sizeof(float))
                    ...
                    for ii in range(sz):
                        for jj in range(sz):
                            ...
                            aa1[ii,jj]=model
                            bb1[ii,jj]=data
                            # plt.imshow(aa1,origin='lower')    
                            # plt.imshow(bb1,origin='lower')  
                            # np.sum((np.abs(aa1-bb1)))
                            if (data>0):
                                newErr = newErr + 2*((model-data)-data*np.log(model/data))
                            else:
