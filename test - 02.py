...
# orign_image=cv2.imread('G:/tony lab/cx/Capture.png')
    ...
    if i==0:
        ...
        Sum_Fixed_distance=0 #The distance from the spot to the center of DH-PSF is calculated
        for pairnum1 in range(pairnum):
            
            Sum_Fixed_distance=distance[idx[pairnum1]]+Sum_Fixed_distance
        Fixed_distance=Sum_Fixed_distance/pairnum/2
        ...
        for j2 in range(len(pair)): #The center of DH-PSF of this frame is calculated
            ...
            if (2*Fixed_distance-20)<math.hypot(xc2 - xc1, yc2 - yc1)<(2*Fixed_distance+20):
                xc=(xc1+xc2)/2
                yc=(yc1+yc2)/2
                pair[j2]=(xc,yc,(ridx[j2][0],ridx[j2][1]),1)
                
                usedspots.append([ridx[j2][0],ridx[j2][1]])
        ...
        if int(len(boxes_ids)/2)>len(pair):
            ...
            for aanum in range(len(a2)):
                for aanum1 in range(len(usedspots2)):
                    if a2[aanum]==usedspots2[aanum1]:
                        contained[aanum] = 1 
            sumcontained=np.sum(contained)
            unusedspots=np.zeros(int(len(a2)-sumcontained))
            jishu=0
            for aanum in range(len(contained)):
                if contained[aanum]==0:
